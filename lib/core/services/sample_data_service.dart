...
class SampleDataService {
  ...
  Future<void> generateSampleData(String userId) async {
    await _generateSampleUsers(userId);
    ...
    await _generateSampleDrawingTemplates();
    await _generateSampleDrawings(userId);
    await _generateSampleTeacherStudentLinks(userId);
  }

  Future<void> _generateSampleUsers(String userId) async {
    // Sample users
    final List<Map<String, dynamic>> sampleUsers = [
      {
        ...
        'address': 'Hà Nội',
        'bio': 'Giáo viên chuyên dạy trẻ tự kỷ với 5 năm kinh nghiệm',
        'specializations': ['Tự kỷ', 'Rối loạn ngôn ngữ', 'Kỹ năng xã hội'],
    ...
    // Add sample students
    final sampleStudentIds = ['student1', 'student2', 'student3'];
    for (final studentId in sampleStudentIds) {
      sampleUsers.add({
        ...
        'dateOfBirth': Timestamp.fromDate(DateTime.now().subtract(const Duration(days: 365 * 6))), // 6 years old
        'gender': 'male',
        'parentId': 'parent1',
        'diagnosis': 'Tự kỷ mức độ nhẹ',
        'interests': ['Vẽ', 'Âm nhạc', 'Động vật'],
        'createdAt': Timestamp.now(),
        'updatedAt': Timestamp.now(),
      });
    }
  ...
  Future<void> _generateSampleDrawingTemplates() async {
    // Sample drawing templates
    final List<Map<String, dynamic>> sampleTemplates = [
      {
        'title': 'Khuôn mặt vui vẻ',
        'description': 'Vẽ một khuôn mặt vui vẻ với nụ cười',
        'category': 'Cảm xúc',
        ...
        'steps': [
          {
            'order': 1,
            'description': 'Vẽ hình tròn cho khuôn mặt',
            'imageUrl': 'https://cdn-icons-png.flaticon.com/512/742/742751.png',
          },
          {
            'order': 2,
            'description': 'Vẽ hai mắt',
            'imageUrl': 'https://cdn-icons-png.flaticon.com/512/742/742751.png',
          },
          {
            'order': 3,
            'description': 'Vẽ nụ cười',
            'imageUrl': 'https://cdn-icons-png.flaticon.com/512/742/742751.png',
          },
        ],
        'createdAt': Timestamp.now(),
      ...
      {
        'title': 'Con mèo đơn giản',
        'description': 'Vẽ một chú mèo dễ thương',
        'category': 'Động vật',
        'difficulty': AppConstants.difficultyMedium,
        'imageUrl': 'https://cdn-icons-png.flaticon.com/512/1818/1818401.png',
        'steps': [
          {
            'order': 1,
            'description': 'Vẽ đầu mèo hình tròn',
            'imageUrl': 'https://cdn-icons-png.flaticon.com/512/1818/1818401.png',
          },
          {
            'order': 2,
            'description': 'Vẽ hai tai tam giác',
            'imageUrl': 'https://cdn-icons-png.flaticon.com/512/1818/1818401.png',
          },
          {
            'order': 3,
            'description': 'Vẽ mắt và mũi',
            'imageUrl': 'https://cdn-icons-png.flaticon.com/512/1818/1818401.png',
          },
          {
            'order': 4,
            'description': 'Vẽ râu mèo',
            'imageUrl': 'https://cdn-icons-png.flaticon.com/512/1818/1818401.png',
          },
        ],
        'createdAt': Timestamp.now(),
    ...
    for (final template in sampleTemplates) {
      await _firestore.collection('drawing_templates').add(template);
    }
  }

  Future<void> _generateSampleDrawings(String userId) async {
    // Get templates
    final templatesSnapshot = await _firestore.collection('drawing_templates').limit(2).get();
    if (templatesSnapshot.docs.isEmpty) return;

    // Sample drawings
    final List<Map<String, dynamic>> sampleDrawings = [];

    for (final templateDoc in templatesSnapshot.docs) {
      sampleDrawings.add({
        'userId': userId,
        'templateId': templateDoc.id,
        'imageUrl': 'https://via.placeholder.com/400x400?text=Sample+Drawing',
        'completionTime': 300, // seconds
        'rating': 5,
        'feedback': 'Rất tốt! Con đã vẽ rất đẹp và đúng các bước.',
        'createdAt': Timestamp.now(),
        'updatedAt': Timestamp.now(),
      });
    }

    // Add drawings to Firestore
    for (final drawing in sampleDrawings) {
      await _firestore.collection('drawings').add(drawing);
    }
  }

  Future<void> _generateSampleTeacherStudentLinks(String userId) async {
    // Get user role
