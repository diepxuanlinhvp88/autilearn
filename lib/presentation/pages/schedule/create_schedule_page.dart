...
class _CreateSchedulePageState extends State<CreateSchedulePage> {
  ...
  List<int> _recurrenceDays = [DateTime.now().weekday];
  DateTime? _recurrenceEndDate;

  bool _hasReminder = false;
  int _reminderMinutesBefore = 30;

  ScheduleType _type = ScheduleType.lesson;
  String _relatedId = '';
        ...
        body: BlocConsumer<ScheduleBloc, ScheduleState>(
          listener: (context, state) {
            if (state is ScheduleError) {
              ...
              ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(
                  content: Text('Đã tạo lịch học mới'),
                  backgroundColor: Colors.green,
                ),
              );

              // Chuyển đến trang chi tiết lịch học
  ...
  void _saveSchedule(BuildContext context) {
    if (_formKey.currentState!.validate()) {
      ...
      if (authState is Authenticated) {
        ...
        context.read<ScheduleBloc>().add(CreateSchedule(schedule));
      }
    }
  }
